...
final class Store: ObservableObject {
    @Published private(set) var items: [FlashtrackItem] = []
    ...
    init() {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Flashtrack", isDirectory: true)
    ...
    func add(title: String, note: String = "", value: Double = 0, date: Date = Date()) -> Bool {
        ...
        let item = FlashtrackItem(title: title, note: note, value: value, date: date)
        items.insert(item, at: 0)
        save()
        return true
    }

    func update(_ item: FlashtrackItem) {
    ...
    func delete(_ item: FlashtrackItem) {
        items.removeAll { $0.id == item.id }
        save()
    }

    func toggleDone(_ item: FlashtrackItem) {
        guard let idx = items.firstIndex(where: { $0.id == item.id }) else { return }
    ...
    private func load() {
        if let data = try? Data(contentsOf: fileURL),
           let decoded = try? JSONDecoder().decode([FlashtrackItem].self, from: data) {
            items = decoded
        } else {
    ...
    static var seedData: [FlashtrackItem] {
        let now = Date()
        let cal = Calendar.current
        return (0..<5).map { i in
            FlashtrackItem(
                title: "Sample Deck \(i + 1)",
